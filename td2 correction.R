...
x2 <- rnorm(nrow(dataf),mean = mean(dataf$rawpoll_trump), sd = sd(dataf$rawpoll_trump))
plot(density(x2))
plot(density(dataf$rawpoll_trump))
ks.test(dataf$rawpoll_trump,x2)

ks.test(dataf$rawpoll_trump,dataf$rawpoll_clinton)
ks.test(scale(dataf$rawpoll_trump),scale(dataf$rawpoll_clinton))
...
library(ggplot2)
ggplot(dataf, aes(x = samplesize, y = rawpoll_trump))+
...
ggplot(dataf, aes(x = samplesize, y = rawpoll_trump))+
  ...
  geom_smooth(aes(color = population, fill = population))+
  facet_wrap(~population, ncol = 4, nrow = 1)


ggplot(dataf, aes(x = samplesize, y = rawpoll_trump))+
  geom_point(aes(color = grade))+               
  geom_smooth(aes(color = grade, fill = grade))+
  facet_wrap(~grade, ncol = 11, nrow = 1)
...
require(graphics)
boxplot(adjpoll_clinton ~ population, data = dataf)
boxplot(adjpoll_trump ~ population, data = dataf)
boxplot(rawpoll_clinton ~ type, data = dataf)

kruskal.test(rawpoll_clinton ~ type, data=dataf)

kruskal.test(adjpoll_clinton ~ population, data = dataf )

kruskal.test(adjpoll_trump ~ population, data = dataf )

kruskal.test(adjpoll_clinton ~ type, data=dataf)
...
# C'est normal : regardez les données et constatez des doublons  !!!!!!!!!!!
## Essayons avec le grade
...
#Exemple de sous séléction
#Estimation des scores de google brut pour Trump

#calcule de l'intervale de confiance pour une distribution t
rowTrumpGoogle <- dataf[ which(dataf$pollster=='Google Consumer Surveys'),]$rawpoll_trump
rowTrumpGoogle 
moyenne <- mean(rowTrumpGoogle)
ecartType <- sqrt(var(rowTrumpGoogle))
n <- length(rowTrumpGoogle)
erreur <- qt(c(1 - 0.05/2) , df=n - 1) * (ecartType / sqrt(n))
left <- moyenne-erreur
right <- moyenne+erreur
left
right


#Google s'est bien planté quand même !