...
toto <-  df[df["Year"] == "2000" & df["Genre"] == "Action", ]
...
#Les jeux de tirs ont l'air d'être un bon investissment mais pour être sur de son intervalle de confiance il faut avoir une distribution gaussienne

sample_data <- df.new[df.new$Genre =="Shooter",]
#centrage réduction
#sample_data$EU_Sales_center <-  scale(sample_data$EU_Sales)

#met à jours la liste des modalitées existantes
sample_data <- droplevels(sample_data)

 boxplot( EU_Sales ~ Publisher, col='yellow', 
                 pch=23, 
                 xlab = "Genre", ylab = "EU_Sales",
                 data=sample_data)
...
summary(sample_data)
summary(sample_data$Publisher)

shapiro.test(sample_data[sample_data$Publisher == "Take-Two Interactive",]$EU_Sales)
#avec un risque accépté de 10% sur la normalité on a trouvé un échantillon gaussien, à vous de voir si vous trouvez mieux 
